...
c check if any particles coming from above or front belong here
  ...
  160 continue
      if (jsr(1,m).ne.0) then
         if (n.eq.1) then
            print*, 'Info:',jsr(1,m),' particles returning above'
         elseif (n.eq.2) then
            print*, 'Info:',jsr(1,m),' particles returning front'
         endif
      endif
c check if any particles coming from below or back belong here
  ...
  170 continue
      if (jss(2,m).ne.0) then
         if (n.eq.1) then
            print*, 'Info:',jss(2,m),' particles returning below'
         elseif (n.eq.2) then
            print*, 'Info:',jss(2,m),' particles returning back'
         endif
      endif
...
c check if move would overflow particle array
  ...
  250 continue
      ...
      ierr = ibflg(1) - npmax
      if (ierr.gt.0) then
         print*, 'particle overflow error, ierr = ', ierr
         info(1) = ierr
         return
      endif
...
c check if any particles have to be passed further
      ...
      if (ibflg(2).gt.0) then
         ! print*, 'Info: particles being passed further = ', ibflg(2)
         ! flush(2)
         if (ibflg(3).gt.0) ibflg(3) = 1
         if (iter.lt.itermax) go to 110
         ierr = -((iter-2)/2)
         print*, 'Iteration overflow, iter = ', ierr
         info(1) = ierr
         go to 320
      endif
...
c debugging section: count total number of particles after move
  ...
  310 continue
      ...
      if (bflg(1).ne.bflg(2)) then
         print*, 'particle number error, old/new=',bflg(1),bflg(2)
         info(1) = 1
      endif
c information
  320 if (nter.gt.0) then
         print*, 'Info: ', nter, ' buffer overflows, nbmax=', nbmax
      endif
      return
      end
c-----------------------------------------------------------------------
      subroutine PMOVE32_tracks(part,edges,npp,sbufr,sbufl,rbufr,rbufl,i
     1hole,jsr,jsl,jss,ny,nz,kstrt,nvpy,nvpz,idimp,npmax,mblok,nblok,idp
     2s,nbmax,idds,ntmax,info,tracks)
c this subroutine moves particles into appropriate spatial regions
c periodic boundary conditions with 2D spatial decomposition
c part(1,n,m) = position x of particle n in partition m
c part(2,n,m) = position y of particle n in partition m
...
c check for full buffer condition
  ...
  100 continue
      ibflg(3) = nps
c ihole has list of indexes of particles that are leaving this processor
c use it to update the missing particle lists for tracking
      do 105 m = 1, mnblok
        call missing_particles(tracks,ihole(1:jss(1,m),m),jss(1,m),part)
  105 continue 
c copy particle buffers
  110 iter = iter + 2
      mter = mter + 1
...
c distribute particles coming from below or back into holes
  ...
  255 continue
      ...
      real_tag = part(addtag_loc,ihole(j,m),m)
		if (int_tag(1) < 0) then
			call new_present_particles(tracks, int_tag(2), ihole(j,m))
		endif
...
c distribute particles coming from above or front into holes
  ...
  263    continue
         ...
         real_tag = part(addtag_loc,ihole(j+jsl(2,m),m),m)
			if (int_tag(1) < 0) then
				call new_present_particles(tracks, int_tag(2), ihole(j+js
     1l(2,m),m))
			endif
...
c distribute remaining particles from below or back into bottom
  ...
  267    continue
         ...
         real_tag = part(addtag_loc,j+npp(m),m)
			if (int_tag(1) < 0) then
				call new_present_particles(tracks, int_tag(2), j+npp(m))
			endif
...
c move particle only if it is below current hole
  ...
  273       continue
				...
				real_tag = part(addtag_loc,ihole(j2,m),m)
				if (int_tag(1) < 0) then
					call change_index(tracks, int_tag(2), ihole(j2,m))
				endif
...
c information
   60 nter = info(n+3)
      if (nter.gt.0) then
         print*, 'Info: ', nter, ' buffer overflows, n, nbmax=', n,
     1nbmax
      endif
      return
      end
...
c-----------------------------------------------------------------------
      subroutine WPMOVES32_tracks(part,edges,npp,sbufr,sbufl,rbufr,rbufl
     1,ihole,jsr,jsl,jss,th,ny,nz,kstrt,nvpy,nvpz,idimp,npmax,mblok,nblo
     2k,idps,nbmax,idds,ntmax,info,tracks)
c wrapper function for particle manager
c info(6) = maximum number of particle passes required in y
c info(6) must be set on entry
c info(7) = maximum number of particle passes required in z
...
c use it to update the missing particle lists for tracking      
   ...
   45 continue
c send outgoing particles
      call PMOVESS32_tracks(part,edges,npp,sbufr,sbufl,rbufr,rbufl,ihole
     1,jsr,jsl,jss,ny,nz,kstrt,nvpy,nvpz,idimp,npmax,mblok,nblok,idps,nb,
     2max,idds,ntmax,info,n,tracks)
c particle or iteration overflow error
      if (info(1).ne.0) return
   40 continue
      return
...
c     info(5+n) = max0(info(5+n),mter)
      if (mter.eq.info(5+n)) then
         if (ibflg(2).gt.0) then
            print*, 'Exceeded maximum number of passes = ', n, mter
            info(1) = -1
            return
         endif
c        print*, 'Info: particles being passed further = ', ibflg(2)
      else
         if (ibflg(3).gt.0) ibflg(3) = 1
         if (iter.lt.itermax) go to 110
         ierr = -((iter-2)/2)
...
c check if buffer overflowed and more particles remain to be checked
      if (ibflg(3).gt.0) then
         nter = nter + 1
         info(3+n) = nter
         print*, 'Buffer overflow = ', n, nter
         info(1) = -2
      endif
      return
      end
c-----------------------------------------------------------------------
      subroutine PMOVESS32_tracks(part,edges,npp,sbufr,sbufl,rbufr,rbufl
     1,ihole,jsr,jsl,jss,ny,nz,kstrt,nvpy,nvpz,idimp,npmax,mblok,nblok,i
     2dps,nbmax,idds,ntmax,info,n,tracks)
c this subroutine moves particles into appropriate spatial regions
c periodic boundary conditions with 2D spatial decomposition
c part(1,n,m) = position x of particle n in partition m
c part(2,n,m) = position y of particle n in partition m
...
c distribute particles coming from above or front into holes
  ...
  360    continue
         ...
         ! update in tracking info
		   real_tag = part(addtag_loc,ihole(j+jsl(2,m),m),m)
		   if (int_tag(1) < 0) then
			   call new_present_particles(tracks,int_tag(2),ihole(j+jsl(2,m
     1),m))
		   endif
...
c process errors
         if (npr.ne.0) then
            ierr = npr
            print*, 'local field overflow error, ierr = ', ierr
            go to 760
         endif
...
c field overflow error
      if (ibflg(1).ne.0) then
         ierr = ibflg(1)
         print*, 'global field overflow error, ierr = ', ierr
         go to 760
      endif
c check if any fields have to be passed further
      if (ibflg(2).gt.0) then
         print*, 'Info: fields being passed further = ', ibflg(2)
         go to 50
      endif
      mnter = nps
      if (n.eq.1) then
