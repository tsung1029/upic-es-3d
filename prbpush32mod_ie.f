...
!
     ...
     &idimp,npmax,mnblok,nxv,nypmx,nxyzp,idds,ipbc)
      ...
      interface
         subroutine PGSRPUSH32F(part,fxyz,npp,noff,qbm,dt,ci,ek,nx,ny,nz,&
     &idimp,npmax,mnblok,nxv,nypmx,nxyzp,idds,ipbc)
         integer :: nx, ny, nz, idimp, npmax, mnblok, nxv, nypmx, nxyzp
         integer :: idds, ipbc
         real :: qbm, dt, ci, ek
     ...
     &idimp,npmax,mnblok,nxv,nypmx,nzpmx,idds,ipbc)
            ...
            if (opt==LOOKAHEAD) then
               if (idimp > 7) then ! if tracking forces for test particles
                  call PGSRPUSH32F(part,fxyz,npp,noff,qbm,dt,ci,ek,nx,ny,nz,&
         &idimp,npmax,mnblok,nxv,nypmx,nxyzp,idds,ipbc)
               else
