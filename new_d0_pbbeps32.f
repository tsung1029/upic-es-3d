...
! non-periodic boundary conditions
      ...
      integer :: m2bmin, m2blok
      integer :: nxhy2z, nxy2zh
      real, dimension(:,:,:,:,:), pointer :: cur, exyzr, bxyzr
! dirichlet boundary conditions
      integer :: l2bmin, l2blok
      real, dimension(:,:,:,:), pointer :: q3, sfield3
      real, dimension(:,:,:,:,:), pointer :: cu3, fxyz3, bxyz3
      complex, dimension(:,:,:,:), pointer :: qt3, qs3, sfieldt3
      complex, dimension(:,:,:,:,:), pointer :: cut3, fxyzt3, bxyzt3
      complex, dimension(:,:,:,:,:), pointer :: fxyzs3
...
! semi-periodic boundary conditions
      integer :: kzyb1, l1bmin, m1bmin
      real, dimension(:,:,:,:), pointer :: q2, sfield2
      real, dimension(:,:,:,:,:), pointer :: cu2, fxyz2, bxyz2
      complex, dimension(:,:,:,:), pointer :: qt2, qs2, sfieldt2
      complex, dimension(:,:,:,:,:), pointer :: cut2, fxyzt2, bxyzt2
      complex, dimension(:,:,:,:,:), pointer :: fxyzs2
...
! non-periodic boundary conditions
      if (psolve /= PERIODIC_3D) then
         allocate(cur(3,nz,kxyp2,kyzp2,j2blok*m2blok))
         allocate(exyzr(3,nz,kxyp2,kyzp2,j2blok*m2blok))
         allocate(bxyzr(3,nz,kxyp2,kyzp2,j2blok*m2blok))
      endif
! dirichlet boundary conditions
      if (psolve==DIRICHLET_3D) then
         ...
         allocate(cu3(3,nx2e,2*nypmx,kzp2*k2bmin,k2blok*l2blok))
         allocate(fxyz3(3,nx2e,2*nypmx,kzp2*k2bmin,k2blok*l2blok))
         allocate(bxyz3(3,nx2e,2*nypmx,kzp2*k2bmin,k2blok*l2blok))
         allocate(sfield3(nx2e,2*nypmx,kzp2*k2bmin,k2blok*l2blok))
         allocate(qt3(nz2,kxyp2,kyzp2*m2bmin,j2blok*m2blok))
         allocate(cut3(3,nz2,kxyp2,kyzp2*m2bmin,j2blok*m2blok))
         allocate(fxyzt3(3,nz2,kxyp2,kyzp2*m2bmin,j2blok*m2blok))
         allocate(bxyzt3(3,nz2,kxyp2,kyzp2*m2bmin,j2blok*m2blok))
...
! semi-periodic boundary conditions
         ...
         allocate(cu2(3,nx2e,2*nypmx,kzp*kbmin,k2blok*lblok))
         allocate(fxyz2(3,nx2e,2*nypmx,kzp*kbmin,k2blok*lblok))
         allocate(bxyz2(3,nx2e,2*nypmx,kzp*kbmin,k2blok*lblok))
         allocate(sfield2(nx2e,2*nypmx,kzp*kbmin,k2blok*lblok))
         allocate(qt2(nz,kxyp2,kyzp2*m1bmin,j2blok*m2blok))
         allocate(cut2(3,nz,kxyp2,kyzp2*m1bmin,j2blok*m2blok))
         allocate(fxyzt2(3,nz,kxyp2,kyzp2*m1bmin,j2blok*m2blok))
         allocate(bxyzt2(3,nz,kxyp2,kyzp2*m1bmin,j2blok*m2blok))
...
! initialize electromagnetic magnetic fields
      ...
      call cguard(bxyze,nyzpu,nx,inorder)
! prepare fft tables
...
! read restart file
     ...
     &)
         ...
         if (ipbc==1) then
            ...
            call rddata(exyzr,nvp,it,ierr)
            if (ierr /= 0) go to 400
            call rddata(bxyzr,nvp,it,ierr)
            if (ierr /= 0) go to 400
         endif
         if (movion==1) then
...
! initialize current density to background
      call sguardp(cu,kstrt,nvpy,nvpz,noff,nyzp,zero,zero,zero,nx,ny,nz,&
     &kblok,ipbc,inorder)
...
! move electrons into appropriate spatial regions
      call pmove(part,edges,npp,tjmove,ny,nz,kstrt,nvpy,nvpz,nbmax,idds,&
     &iblok,vect,ierr)
      if (ierr /= 0) then
         call MP_END
         call PPEXIT
...
! move ions into appropriate spatial regions
         call pmove(parti,edges,nppi,tjmovi,ny,nz,kstrt,nvpy,nvpz,nbmax,&
     &idds,iblok,vect,ierr)
!        call pmove(parti,edges,nppi,tjmovi,ny,nz,kstrt,nvpy,nvpz,nbmax,&
!    &idds,iblok,info,tinfo,ierr)
...
! add guard cells for current in x
      call aguardp(cu,nyzp,nx,ipbc,inorder)
! add guard cells for current in y and z
      call pnaguardp(cu,nyzp,kstrt,nvpy,nvpz,nx,mterg,nterg,ngds,ipbc,kb&
     &lok,inorder)
! add guard cells for density in x
...
! move current to uniform field partition
      isign = -1
      call pfmove(cu,noff,nyzp,isign,tfmove,kyp,kzp,kstrt,nvpy,nvpz,kblo&
     &k,mterf,nterf,ierr,inorder)
      if (ierr /= 0) then
         call MP_END
         call PPEXIT
...
! transform current to fourier space
      call trpsin(cu,cu3,nx,ny,nz,kstrt,nvpy,nvpz,kyp,kyp2,kzp,kzp2,kblo&
     &k,k2blok,inorder)
      isign = -1
      call fft(cu3,fxyzs3,cut3,isign,mixup3,sct3,tfft,indx1,indy1,indz1,&
     &kstrt,kxyp2,kyp2,kyzp2,kzp2,k2blok,m2blok,LINEAR)
! take transverse part of current
      call cuperpd(cut3,nx,ny,nz,kstrt,j2blok)
      call cmfieldd(cut3,cur,nx,ny,nz,kstrt,j2blok)
! calculate magnetic field in fourier space
      if (itime==0) then
         call ibpoisd(cur,bxyzr,ffd,ci,wm,nx,ny,nz,kstrt,j2blok)
         wf = 0.
      else
         call maxweld(exyzr,bxyzr,cur,ffd,affp,ci,dt,wf,wm,nx,ny,nz,kstr&
     &t,j2blok)
      endif
...
!           call bpoisd(cut3,bxyzt3,ffd,ci,wm,nx,ny,nz,kstrt,j2blok)
            call avpotd(bxyzr,cur,nx,ny,nz,kstrt,j2blok)
! copy vector potential
            call cpfieldd(bxyzt3,cur,nx,ny,nz,kstrt,j2blok)
! transform vector potential to real space
            isign = 1
            call fft(bxyz3,fxyzs3,bxyzt3,isign,mixup3,sct3,tfft,indx1,in&
     &dy1,indz1,kstrt,kxyp2,kyp2,kyzp2,kzp2,k2blok,m2blok,LINEAR)
            call haftrp(vfield,bxyz3,nx,ny,nz,kstrt,nvpy,kyp,kyp2,kzp,kz&
     &p2,kblok,k2blok,inorder)
            call pcguardp(vfield,kstrt,nvpy,nvpz,nx,kyp,kzp,ngds,ipbc,kb&
     &lok,inorder)
            call cguardp(vfield,nyzpu,nx,ipbc,inorder)
         endif
      endif
...
! add longitudinal and transverse electric fields
      isign = 1
      call emfieldd(fxyzt3,exyzr,ffd,isign,nx,ny,nz,kstrt,j2blok)
! copy magnetic field
      isign = -1
      call emfieldd(bxyzt3,bxyzr,ffd,isign,nx,ny,nz,kstrt,j2blok)
...
! transform magnetic field to real space
      isign = 1
      call fft(bxyz3,fxyzs3,bxyzt3,isign,mixup3,sct3,tfft,indx1,indy1,in&
     &dz1,kstrt,kxyp2,kyp2,kyzp2,kzp2,k2blok,m2blok,LINEAR)
      call haftrp(bxyze,bxyz3,nx,ny,nz,kstrt,nvpy,kyp,kyp2,kzp,kzp2,kblo&
     &k,k2blok,inorder)
!
! semi-periodic boundary conditions
!
...
! transform current to fourier space
      call dblsin(cu,cu2,nx,ny,kstrt,nvpy,kyp,kyp2,kzp,kblok,k2blok,inor&
     &der)
      isign = -1
      call fft(cu2,fxyzs2,cut2,isign,mixup2,sct2,tfft,indx1,indy1,indz,k&
     &strt,kxyp2,kyp2,kyzp2,kzp,k2blok,m2blok,LINEAR)
! take transverse part of current
      call cuperpm(cut2,nx,ny,nz,kstrt,j2blok)
      call cmfieldm(cut2,cur,nx,ny,nz,kstrt,j2blok)
! calculate magnetic field in fourier space
      if (itime==0) then
         call ibpoism(cur,bxyzr,ffb,ci,wm,nx,ny,nz,kstrt,j2blok)
         wf = 0.
      else
         call maxwelm(exyzr,bxyzr,cur,ffb,affp,ci,dt,wf,wm,nx,ny,nz,kstr&
     &t,j2blok)
      endif
! vector potential diagnostic
      if (nta > 0) then
         ...
         if (itime==nta*it) then
! calculate vector potential in fourier space
!           call bpoism(cut2,bxyzt2,ffb,ci,wm,nx,ny,nz,kstrt,j2blok)
            call avpotm(bxyzr,cur,nx,ny,nz,kstrt,j2blok)
! copy vector potential
            call cpfieldm(bxyzt2,cur,nx,ny,nz,kstrt,j2blok)
! transform vector potential to real space
            isign = 1
            call fft(bxyz2,fxyzs2,bxyzt2,isign,mixup2,sct2,tfft,indx1,in&
     &dy1,indz,kstrt,kxyp2,kyp2,kyzp2,kzp,k2blok,m2blok,LINEAR)
            call hafdbl(vfield,bxyz2,nx,ny,kstrt,nvpy,nvpz,kyp,kyp2,kzp,&
...
! add longitudinal and transverse electric fields
      isign = 1
      call emfieldm(fxyzt2,exyzr,ffb,isign,nx,ny,nz,kstrt,j2blok)
! copy magnetic field
      isign = -1
      call emfieldm(bxyzt2,bxyzr,ffb,isign,nx,ny,nz,kstrt,j2blok)
! transform force/charge to real space
      isign = 1
...
! transform magnetic field to real space
      isign = 1
      call fft(bxyz2,fxyzs2,bxyzt2,isign,mixup2,sct2,tfft,indx1,indy1,in&
     &dz,kstrt,kxyp2,kyp2,kyzp2,kzp,k2blok,m2blok,LINEAR)
      call hafdbl(bxyze,bxyz2,nx,ny,kstrt,nvpy,nvpz,kyp,kyp2,kzp,kblok,k&
     &2blok,inorder)
!
...
! move magnetic field to non-uniform field partition
      isign = 1
      call pfmove(bxyze,noff,nyzp,isign,tfmove,kyp,kzp,kstrt,nvpy,nvpz,k&
     &blok,mterf,nterf,ierr,inorder)
      if (ierr /= 0) then
         call MP_END
         call PPEXIT
...
! copy to guard cells
      call pncguardp(bxyze,nyzp,kstrt,nvpy,nvpz,nx,mterg,nterg,ngds,ipbc&
     &,kblok,inorder)
      call cguardp(bxyze,nyzp,nx,ipbc,inorder)
! external pump
      ...
      if (ntr > 0) then
         ...
         if (itime==ntr*it) then
            ...
            if (ipbc==1) then
              ...
              call wrdata(exyzr,nvp,it)
              call wrdata(bxyzr,nvp,it)
            endif
